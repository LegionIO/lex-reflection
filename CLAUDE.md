...
lib/legion/extensions/reflection/
  ...
  helpers/
    ...
    llm_enhancer.rb      # LlmEnhancer module — optional LLM observation enhancement + dream reflection
  runners/
...
`Helpers::LlmEnhancer` provides optional LLM-driven observation text and dream reflection via `legion-llm`.

**System prompt theme**: Metacognitive reflection engine for an autonomous AI agent. Analyzes post-tick cognitive metrics analytically, identifies correlations between categories, writes as internal first-person reflection rather than a report.

| Method | Parameters | Returns |
|---|---|---|
| `available?` | — | `true` when `Legion::LLM.started?` |
| `enhance_reflection` | `monitors_data:, health_scores:` | `{ observations: { category_sym => "text", ... } }`, or `nil` on failure |
| `reflect_on_dream` | `dream_results:` | `{ reflection: "..." }`, or `nil` on failure |

`enhance_reflection` maps six prompt labels (EMOTION, PREDICTION, MEMORY, TRUST, CURIOSITY, IDENTITY) to their category symbols via `CATEGORY_LABEL_MAP`. Only the `observation` text on each reflection entry is replaced — `severity` and `recommendation` symbols are never touched by LLM.

`reflect_on_dream` generates a 2-4 sentence first-person reflection on a completed dream cycle. Used by the new `reflect_on_dream` runner method.

**Fallback**: When LLM is unavailable or returns nil, `reflect` uses the existing template observation strings from `Helpers::Monitors`. `reflect_on_dream` falls back to `build_mechanical_dream_reflection`, which assembles a sentence from memory audit / contradiction / agenda counts.

**Source indicator**: `reflect_on_dream` returns `source: :llm` or `source: :mechanical`. Individual reflections within `reflect` get `source: :llm` on their entry hash when enhanced.

**CRITICAL**: Recommendation symbols (`:increase_curiosity`, `:stabilize_emotion`, etc.) are never replaced by LLM output. Only the human-readable `observation` string is enhanced.
...
| `reflect_on_dream` | `dream_results: {}` | `{ reflection: String, source: :llm | :mechanical }` |
...
- `reflect_on_dream` is called from `lex-dream`'s `dream_reflection` phase (phase 7) if `lex-reflection` is loaded
...
- LLM enhancement is always optional: `LlmEnhancer` rescues all `StandardError`, logs a warn, and returns nil — template fallback activates automatically
- `LlmEnhancer.available?` also rescues `StandardError` (returns false), so missing `legion-llm` gem never raises
- Recommendation symbols are generated by `Helpers::Monitors` (mechanical) and are never overwritten by LLM output
